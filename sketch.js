  ...
  function draw() {
    ...
    wordSpacing(10 * movement)*.25;
    letterSpacing(10 * movement)*.25;
    text(txt,0,0)
    pop()
    ...
    pop()
      ...
      const spacer = floor((width - margins - textWidth(lineOfWords)) / words);
    ...
    linesOfWords.forEach((lineOfWords, index) => {
      ...
      const wordsSpacer = floor((width - margins - textWidth(lineOfWords)) / words);
      const lettersSpacer = floor((width - margins - textWidth(lineOfWords)) / letters);
      wordSpacing(wordsSpacer * .25 * movement);
      letterSpacing(lettersSpacer* .75 * movement);
      text(lineOfWords, width / 2, height / 2 + textLeading() / 2 + textLeading() * (index-1));
      push()
      noFill()
      translate(width / 2,height / 2 + textLeading() / 2 + textLeading() * (index-1))
      rect(0,0,textWidth(lineOfWords),textLeading())
      pop()
      pop();
    });
